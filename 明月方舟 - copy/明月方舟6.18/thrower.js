...
class Thrower extends Tower {
  ...
  activateSkill(){
            ...
            if (this.currentMana >= this.maxMana && !this.isSkillActive) {
              if (!this.baseDamage) {
                this.baseDamage = this.damage; // 存储基础属性
              }
              // 初始化技能状态（新增）
              this.isSkillActive = true;  // 标记技能激活
              this.bulletsLeft = 6;       // 设置初始子弹数（与 thrower.js 中的耗尽逻辑一致）
              
              // 原有技能增强逻辑
              this.damage = this.baseDamage * 3.7;
              this.secondaryDamage = this.damage * 0.5; // 更新余震伤害
              this.aoeRadius = 180;
              this.shootInterval = 3900;
              this.explosionChance = 1; // 技能开启后爆炸概率100%
              
              // 新增：部署两个召唤物
              this.deploySummoner(); // 部署第一个召唤物
              this.deploySummoner(); // 部署第二个召唤物
