class Enemy {
  constructor(x, y, health, speed, value, color, path, game) {
    this.defense = 3; // 敌人基础防御力
    this.x = x;
    this.y = y;
    ...
    // 新增：烧伤相关属性
    this.burnLayers = 0;         // 当前烧伤层数（x）
    this.isBurning = false;      // 是否处于燃烧状态（标记）
    this.burnStartTime = 0;      // 进入燃烧状态的时刻（毫秒）
  }
  ...
  update(deltaTime, towers = [], gameTime = 0) {
    ...
    if (this.burnLayers > 0) {
      ...
      if (burnDuration >= 4000) { // 4秒（4000毫秒）
        // 计算并扣除伤害（150 * 当前层数）
        const burnDamage = 150 * this.burnLayers;
    ...
    // 新增：检查是否被任意地面塔阻挡（存在于至少一个塔的blockedEnemies中）
      ...
      if (!this.hasInitialBlockOffset) { 
        ...
        if (blockingTower) {
          ...
          switch (towerDirection) {
            case 0: // 塔方向：右（朝右）
              switch (enemyIndex) {
                case 0: this.blockOffsetX = 5; this.blockOffsetY = 0; break;  // 第1个敌人：右偏移
                case 1: this.blockOffsetX = 0; this.blockOffsetY = 0; break;  // 第2个敌人：无偏移
                case 2: this.blockOffsetX = -5; this.blockOffsetY = 0; break; // 第3个敌人：左偏移
              }
              break;
            case 1: // 塔方向：下（朝下）
              switch (enemyIndex) {
                case 0: this.blockOffsetX = 0; this.blockOffsetY = 5; break;  // 第1个敌人：下偏移
                case 1: this.blockOffsetX = 0; this.blockOffsetY = 0; break;  // 第2个敌人：无偏移
                case 2: this.blockOffsetX = 0; this.blockOffsetY = -5; break; // 第3个敌人：上偏移
              }
              break;
            case 2: // 塔方向：左（朝左）
              switch (enemyIndex) {
    ...
    if (!this.isStunned) {
      ...
      for (const tower of towers) {
        if (tower.type === 'ground' && tower.blockedEnemies.includes(this)) {
      ...
      if (this.targetTower) {
        if (!this.attackCooldown) {
          ...
          if (this.targetTower.defense >= this.attackDamage) {
            finalDamage = Math.floor(this.attackDamage * 0.05);
          } else {
            finalDamage = Math.max(1, this.attackDamage - this.targetTower.defense);
          }
          this.targetTower.health -= finalDamage;
