class ShaoTower extends Tower {
    ...
    if (this.type === 'shao') {
      ...
      for (const enemy of targets) {
        if (enemy.health > 0) {
          ...
          enemy.addBurnLayers(addedLayers);
          // 施加星火状态
          enemy.spark = true;
  ...
  // 修改：使用累计运行时间计算25秒间隔（暂停时runningTime不增加）
  if (this.runningTime - this.moodcointime >= 25000) {
    this.moodcoin += 1;
    this.moodcointime = this.runningTime; // 重置为当前累计运行时间
  }
  ...
  onEgoState(deltaTime, enemies, bullets) {
    ...
    if (this.isEgoActive && !this.egoBurnInterval) {
      this.egoBurnInterval = setInterval(() => {
        ...
        game.enemies.forEach((enemy) => {
          // 为每个敌人添加 1 层烧伤
          enemy.addBurnLayers(2);
          // 若敌人未处于燃烧状态且烧伤层数大于 0，将其设为燃烧状态并记录开始时间
        });
      }, 3000);
    }
    // 暂停时停止定时器，恢复时重启定时器 (新增状态判断)
    
    // 若EGO激活且游戏正常运行，启动定时器
    if (this.isEgoActive && this.game.normalSpeed) {
      this.startEgoBurnTimer();
    } else {
      // 暂停或EGO关闭时停止定时器
      this.stopEgoBurnTimer();
    }
    }
    ...
    startEgoBurnTimer() {
      if (!this.egoBurnInterval) {
        this.egoBurnInterval = setInterval(() => {
          ...
          game.enemies.forEach((enemy) => {
            enemy.addBurnLayers(2);
            if (!enemy.isBurning && enemy.burnLayers > 0) {
              enemy.isBurning = true;
              enemy.burnStartTime = game.gameTime;
            }
          });
        }, 3000);
     }
    ...
    stopEgoBurnTimer() {
      if (this.egoBurnInterval) {
        clearInterval(this.egoBurnInterval);
        this.egoBurnInterval = null;
      }
    }
