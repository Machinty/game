class Enemy {
  constructor(x, y, health, speed, type, color, path, game,defense) {
    ...
    // 新增：本次伤害结算前获得烧伤（待结算烧伤层数）
    this.pendingBurnLayers = 0;  // 从上一次结算后到下一次结算前获得的总烧伤层数  
    // 原有烧伤伤害显示属性（未修改）
  ...
  update(deltaTime, towers = [], gameTime = 0) {
    ...
    if (this.burnLayers > 0) {
      ...
      const minInterval = 1500; // 1.5秒（毫秒）
      const maxInterval = 4000; // 4秒（毫秒）
      ...
      if (burnDuration >= interval) { // 使用动态间隔替代固定4秒
        ...
        this.pendingBurnLayers = 0; // 重置待结算层数
      ...
      this.game.enemies.forEach(enemy => {
        ...
        if (distance <= 60) {
          ...
          const transferLayers = Math.floor(this.burnLayers / 2);
            if (transferLayers > 0) {
                      // 检查待结算烧伤层数是否大于0
              if (enemy.pendingBurnLayers > 0) {
                        // 计算实际可传递的层数（不超过待结算层数）
                const actualTransfer = Math.min(transferLayers, enemy.pendingBurnLayers);
                        // 施加等量烧伤
                enemy.burnLayers += actualTransfer;
                        // 扣除待结算层数
                enemy.pendingBurnLayers -= actualTransfer;
                        
                        // 确保燃烧状态激活
                if (!enemy.isBurning) {
                enemy.isBurning = true;
                enemy.burnStartTime = gameTime;
                        }
                      }
        }
      };
   ...
   */
     physicalAttack(initialDamage,defence) {
      return Math.max(initialDamage -defence, initialDamage * 0.05);
    }
  
    addBurnLayers(amount) {
      this.burnLayers += amount;
      this.pendingBurnLayers += amount;
    }
